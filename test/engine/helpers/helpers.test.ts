...
describe("Helpers", () => {
  ...
  describe("isOverDelegated", () => {
    ...
    test("returns `false` if the baker's stake is gte to 10% of the staking balance", () => {
      ...
      expect(isOverDelegated(bakerBalance, stakingBalance, null)).toEqual(
        false
      );
    });
  ...
  describe("getMinimumDelegationAmount", () => {
    it("returns the minimum delegation amount if it is defined", () => {
      const minimumDelegationBalance = 100;
      const config = generateConfig({
        delegator_requirements: {
          minimum_balance: minimumDelegationBalance,
        },
      });
      const result = getMinimumDelegationAmount(config);
      expect(result).toStrictEqual(new BigNumber(minimumDelegationBalance));
    });
